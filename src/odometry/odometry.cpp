...
void aekulib::Odometry::wheel_distance(inches<>& left_dist, inches<>& right_dist, inches<>& back_dist){
...
x_change = (angle_change == 0) ? back_distance : 2 * std::sin(angle_change / 2) * (((back_distance) / angle_change) + Ts);
y_change = (angle_change == 0) ? right_distance : 2 * std::sin(angle_change / 2) * (((right_distance)/ angle_change) + Tr);
    ...
    x_coord = x_coord + x_change_correct;
    y_coord = y_coord + y_change_correct;
