...
namespace aekulib
{
    ...
    {
        m_motorGroup->move_voltage(voltage.to<int32_t>());
    }
    ...
    {
        m_motorGroup->set_brake_mode(mode);
    }
    ...
    {
        return degrees(m_motorGroup->get_position());
    }
    ...
    amperes<> MotorGroup::getCurrentDraw() const
    {
        return milliamperes(m_motorGroup->get_current_draw());
    }

    std::int32_t MotorGroup::getDirection() const
    {
        return m_motorGroup->get_direction();
    }

    celsius<> MotorGroup::getTemperature() const
    {
        return celsius(m_motorGroup->get_temperature());
    }

    double MotorGroup::getEfficiency() const { return m_motorGroup->get_efficiency(); }

    newton_meters<> MotorGroup::getTorque() const
    {
        return newton_meters(m_motorGroup->get_torque());
    }
