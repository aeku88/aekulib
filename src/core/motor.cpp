...
namespace aekulib
{
    ...
    {
        m_motor->move_voltage(voltage.to<int32_t>());
    }
    ...
    {
        m_motor->set_brake_mode(mode);
    }

    void Motor::resetPosition() const { m_motor->tare_position(); }

    degrees<> Motor::getPosition() const
    {
        return degrees(m_motor->get_position());
    }
    ...
    amperes<> Motor::getCurrentDraw() const
    {
        return milliamperes(m_motor->get_current_draw());
    }

    std::int32_t Motor::getDirection() const
    {
        return m_motor->get_direction();
    }

    celsius<> Motor::getTemperature() const
    {
        return celsius(m_motor->get_temperature());
    }

    double Motor::getEfficiency() const { return m_motor->get_efficiency(); }

    newton_meters<> Motor::getTorque() const
    {
        return newton_meters(m_motor->get_torque());
    }
