...
namespace squiggles
{
    class SplineGenerator
    {
      ...
      public:
        ...
        {
            ConstrainedState(Pose ipose, double icurvature, double idistance, double imax_vel,
                             double imin_accel, double imax_accel)
                : pose(ipose), curvature(icurvature), distance(idistance), max_vel(imax_vel),
                  min_accel(imin_accel), max_accel(imax_accel)
            {}
      ...
      public:
        ...
        parameterize(const ControlVector start, const ControlVector end,
                     const std::vector<GeneratedPoint> &raw_path, const double preferred_start_vel,
         ...
         */
        ProfilePoint lerp_point(QuinticPolynomial x_qp, QuinticPolynomial y_qp, ProfilePoint start,
                                ProfilePoint end, double i);

        /**
         * Returns the spline curve for the given control vectors and path duration.
         */
        QuinticPolynomial
        get_x_spline(const ControlVector start, const ControlVector end, const double duration);
        QuinticPolynomial
