    ...
    inline namespace v5
    {
        class MotorGroup : public virtual AbstractMotor
        {
          ...
          public:
             ...
             *
             * \param port
             *        A initializer list of V5 port numbers from 1 to 21, or from -21 to -1 for reversed
             * motors. A reversed motor will reverse the input or output movement functions and movement
             * related telemetry in order to produce consistant behavior with non-reversed motors
             *
             ...
             * EDOM - The motor group is empty
             *
             * \return A vector containing the following for each motor: 1 if the motor's current limit is
             * being exceeded and 0 if the current limit is not exceeded, or PROS_ERR if the operation failed,
             * setting errno.
             *
             * \b Example
             * \code
             ...
             * EDOM - The motor group is empty
             *
             * \return A vector with one of MotorBrake for each motor in the motor group, according to what
             * was set for the motor, or E_MOTOR_BRAKE_INVALID if the operation failed, setting errno.
             *
             * \b Example
             * \code
             * void initialize() {
             ...
             * EDOM - The motor group is empty
             *
             * \return A vector with the following for each motor, One of MotorUnits according to what is set
             ...
             * EDOM - The motor group is empty
             *
             * \return A vector conatining the following for each motor: 1 if the motor has been reversed and
             * 0 if the motor was not reversed, or PROS_ERR if the operation failed, setting errno.
             *
             * \b Example
             * \code
             * void initialize() {
             ...
             */
            std::int32_t
            set_brake_mode(const pros::motor_brake_mode_e_t mode, const std::uint8_t index = 0) const;
            /**
             ...
             */
            std::int32_t
            set_encoder_units(const pros::motor_encoder_units_e_t units, const std::uint8_t index = 0) const;
            /**
             ...
             * 	only the first n motors will have their gearsets changed where n is the number of gearsets
             * passed in. If there are more gearsets passed in than motors, then the only the first m gearsets
             * will be used, where m is the number of motors. In either case, errno will be set to E2BIG, but
             * the operation still occurs
             *
             *
             * \param gearset
             ...
             */
            std::int32_t
            set_gearing(const pros::motor_gearset_e_t gearset, const std::uint8_t index = 0) const;

            /**
             * Sets the gear cartridge (red, green, blue) for each motor in the motor group by taking in a
             * vector of the cartridges. Usable with the C++ enum class and the C enum.
             *
             * This function uses the following values of errno when an error state is
             * reached:
             * ENODEV - The port cannot be configured as a motor
            ...
            /**
             * Sets one of the gear cartridge (red, green, blue) for every motor in the motor group. Usable
